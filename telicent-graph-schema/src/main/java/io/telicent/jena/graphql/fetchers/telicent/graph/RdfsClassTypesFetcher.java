...
/**
 * Fetches rdf:type values for a class.
 */
public class RdfsClassTypesFetcher implements DataFetcher<List<TelicentGraphNode>> {

    /**
     * Creates a new fetcher.
     */
    public RdfsClassTypesFetcher() {
    }
    ...
    public List<TelicentGraphNode> get(DataFetchingEnvironment environment) {
        ...
        return Txn.calculateRead(dsg, () -> {
            Stream<Node> stream = dsg.stream(graphNode, classNode, RDF.type.asNode(), Node.ANY)
                                     .map(Quad::getObject)
