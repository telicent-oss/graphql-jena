...
/**
 * Fetches the range class for an object property.
 */
public class ObjectPropertyRangeFetcher implements DataFetcher<RdfsClass> {

    /**
     * Creates a new fetcher.
     */
    public ObjectPropertyRangeFetcher() {
    }
    ...
    public RdfsClass get(DataFetchingEnvironment environment) {
        ...
        return Txn.calculateRead(dsg, () -> {
            Optional<Node> range = dsg.stream(graphNode, predicate, RDFS.range.asNode(), Node.ANY)
                                      .map(Quad::getObject)
                                      .findFirst();
            boolean owlClass = range.filter(node -> dsg.contains(graphNode, node, RDF.type.asNode(),
                                                                OWL.Class.asNode()))
                                    .isPresent();
            return range.map(node -> new RdfsClass(node, dsg.prefixes(), owlClass, graphNode))
                        .orElse(null);
        });
    }
}
