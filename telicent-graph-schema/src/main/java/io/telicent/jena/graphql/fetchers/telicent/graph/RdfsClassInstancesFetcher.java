...
/**
 * Fetches instances of a class.
 */
public class RdfsClassInstancesFetcher implements DataFetcher<List<TelicentGraphNode>> {

    /**
     * Creates a new fetcher.
     */
    public RdfsClassInstancesFetcher() {
    }
    ...
    public List<TelicentGraphNode> get(DataFetchingEnvironment environment) {
        ...
        return Txn.calculateRead(dsg, () -> {
            Stream<Node> stream = dsg.stream(graphNode, Node.ANY, RDF.type.asNode(), classNode)
                                     .map(Quad::getSubject)
            ...
            return stream.map(node -> new TelicentGraphNode(node, dsg.prefixes()))
