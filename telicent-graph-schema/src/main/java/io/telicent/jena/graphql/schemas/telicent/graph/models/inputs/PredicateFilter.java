 ...
 */
public class PredicateFilter extends AbstractFilter {
    ...
    public Stream<Quad> filter(Stream<Quad> stream, DatasetGraph dsg) {
        return switch (mode) {
            case INCLUDE -> stream.filter(q -> this.values.contains(q.getPredicate()));
            case EXCLUDE -> stream.filter(q -> !this.values.contains(q.getPredicate()));
        };
    }
