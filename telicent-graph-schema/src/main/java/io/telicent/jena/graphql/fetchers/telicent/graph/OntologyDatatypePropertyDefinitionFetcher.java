...
/**
 * Fetches a single datatype property definition by URI.
 */
public class OntologyDatatypePropertyDefinitionFetcher implements DataFetcher<DatatypePropertyDefinition> {

    /**
     * Creates a new fetcher.
     */
    public OntologyDatatypePropertyDefinitionFetcher() {
    }

    @Override
    public DatatypePropertyDefinition get(DataFetchingEnvironment environment) {
        ...
        return Txn.calculateRead(dsg, () -> {
            if (!dsg.contains(graphNode, propertyNode, RDF.type.asNode(), OWL.DatatypeProperty.asNode())) {
                return null;
            }
            return new DatatypePropertyDefinition(propertyNode, dsg.prefixes(), graphNode);
        });
    }
}
